...
- 개체 수가 바뀌면 1)해당 프레임의 원본 이미지, 2)해당 프레임의 오토인코더 및 컨투어 적용 이미지, 3) 이전 프레임의 원본 이미지, 4) 이전 프레임의 오토인코더 및 컨투어 적용 이미지, 5) 해당 프레임의 개체 수, 6) 이전 프레임의 개체 수를 구글 드라이브에 업로드합니다.<br><br>

[서버]
- 라즈베리파이가 구글 드라이브에 업로드한 파일들을 다운로드 합니다.
- 다시 4개의 이미지로 만들고, 개체 수 정보는 txt파일에 저장합니다.
- 플라스크를 사용해 웹 사이트에서 이미지들과 개체 수를 볼 수 있도록 했습니다. 검색 기능도 포함합니다.
